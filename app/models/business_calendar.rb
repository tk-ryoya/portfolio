class BusinessCalendar
  ...
  def self.temporary_closed_day
    ...
    calendar = Calendar.new
    closed_days = calendar.closed_days
    closed_day = closed_days.map { |closed_day| closed_day.strftime("%m月 %d日 (#{wd[time.wday]})") }
  end
  ...
  def self.temporary_closed_day_pm
    ...
    calendar = Calendar.new
    closed_days_pm = calendar.closed_days_pm
    closed_day_pm = closed_days_pm.map { |closed_day_pm| closed_day_pm.strftime("%m月 %d日 (#{wd[time.wday]})") }
  end
end
