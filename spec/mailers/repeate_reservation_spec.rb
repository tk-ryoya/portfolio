...
RSpec.describe RepeateReservationMailer, type: :mailer do
  describe '管理者へ予約完了の通知メール' do
    let(:user) { create(:user) }
    let(:reservation) { create(:reservation, user: user) }
    let(:repeate_interview) { create(:repeate_interview) }
    let(:mail) { RepeateReservationMailer.reservation_notification(reservation, user, repeate_interview.description) }
    wd = ["日", "月", "火", "水", "木", "金", "土"]
    ...
    it 'メール本文に予約詳細が表示されていること' do
      expect(mail.html_part.body.to_s).to match(repeate_interview.description)
    end
  end
end
